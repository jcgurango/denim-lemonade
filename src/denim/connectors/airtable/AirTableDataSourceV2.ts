import {
  AirTable, AirTableColumn,
} from './types/schema';
import DenimDataSourceV2 from '../../service/DenimDataSourceV2';
import { DenimRecord, DenimQuery, DenimSchema, DenimColumnType, DenimSelectProperties, DenimQueryOperator, DenimQueryConditionOrGroup, DenimQueryConditionGroup, DenimTable } from '../../core';
...
export default class AirTableDataSourceV2 extends DenimDataSourceV2 {
  ...
  private mapDenimRecordToAirTableFields(table: string, record: DenimRecord): any {
  ...
  private mapAirtableToDenimRecord(table: string, record: Record): DenimRecord {
    const airtableSchema = this.airtableSchema.find(({ name, id }) => (name === table || id === table));
  ...
  private conditionToFormula(tableSchema: DenimTable, condition: DenimQueryConditionOrGroup): string {
  ...
  private conditionGroupToFormula(tableSchema: DenimTable, condition: DenimQueryConditionGroup): string {
  ...
  protected async query(table: string, query?: DenimQuery): Promise<DenimRecord[]> {
    ...
    if (query?.conditions) {
      params.filterByFormula = this.conditionToFormula(tableSchema, query.conditions);
    }
    ...
    return records.map((record) => this.mapAirtableToDenimRecord(table, record));
  }

  protected async save(table: string, record: DenimRecord): Promise<DenimRecord> {
    if (record.id) {
      const atRecord = await this.base.table(table).update(
        record.id,
        this.mapDenimRecordToAirTableFields(table, record),
      );

      return this.mapAirtableToDenimRecord(table, atRecord);
    }

    const atRecord = await this.base.table(table).create(
      this.mapDenimRecordToAirTableFields(table, record),
    );
